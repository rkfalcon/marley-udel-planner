...
const TOTAL_CREDITS_REQUIRED = MARLEY_PROFILE.totalCreditsRequired; // 124
...
export function GraduationTracker({ plan }: GraduationTrackerProps) {
  ...
  const estimatedGrad = useMemo(() => {
    ...
    // Find the last semester with planned courses
  ...
  const requirementSummary = useMemo(() => {
    const allReqs = totalCompleted + totalInProgress + totalNotStarted;
    return { met: totalCompleted, inProgress: totalInProgress, total: allReqs };
        ...
        <div>
          ...
          <div className="h-3 rounded-full bg-slate-100 overflow-hidden flex">
            ...
            <div
              ...
              style={{
                width: `${Math.min(100, (completedCredits / TOTAL_CREDITS_REQUIRED) * 100)}%`,
              }}
            />
            {/* In progress */}
            <div
              ...
              style={{
                width: `${Math.min(100, (inProgressCredits / TOTAL_CREDITS_REQUIRED) * 100)}%`,
              }}
            />
            {/* Planned */}
            <div
              ...
              style={{
                width: `${Math.min(100, (plannedCredits / TOTAL_CREDITS_REQUIRED) * 100)}%`,
              }}
            />
          </div>
          ...
          {!isOnTrack && creditsNeeded > 0 && (
            <div className="flex items-center gap-1.5 mt-2 p-2 rounded-md bg-amber-50 border border-amber-200">
              ...
              <span className="text-[11px] text-amber-700">
                Need {creditsNeeded} more credit{creditsNeeded !== 1 ? 's' : ''} to graduate
              </span>
            </div>
          )}
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-slate-500">
              <span className="font-bold text-slate-700">
                {requirementSummary.met}
              </span>
              /{requirementSummary.total} met
            </span>
          </div>

          <div className="space-y-3">
            {groups.map((group) => {
              const completed = group.requirements.filter(
                (r) => r.status === 'completed'
              ).length;
              const inProg = group.requirements.filter(
              ...
              return (
                <div key={group.category}>
                  ...
                  {/* Mini requirement items */}
                  <div className="space-y-0.5 pl-1">
                    {group.requirements.slice(0, 5).map((req) => (
                      <div
                        key={req.id}
                      ...
                      >
                        ...
                        <span
                          className={cn('text-[10px] truncate', {
                          ...
                          title={req.name}
                        >
                          {req.name.replace(/^[A-Z]+\s\d+\w*\s[-–]\s/, '')}
                        </span>
                      </div>
                    ))}
                    {group.requirements.length > 5 && (
                      <span className="text-[10px] text-slate-400 pl-4">
                        +{group.requirements.length - 5} more
                      </span>
                    )}
                  </div>
                </div>
