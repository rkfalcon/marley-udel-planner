...
export function CompletePathway({plan, print=false, compact=false, onChange}: {plan:Plan; print?:boolean; compact?:boolean; onChange?:(pathway:CompleteSlpPathway)=>void}) {
  ...
  return <section aria-label="Complete BS and MA pathway" className="m-4 rounded-xl border border-blue-200 bg-white p-5 text-sm text-slate-700 space-y-4">
    <h2 className="text-xl font-bold text-blue-900">One pathway · B.S. Cognitive Science + M.A. Speech-Language Pathology</h2>
    ...
    <details open={print}><summary className="cursor-pointer font-semibold">M.A. requirement audit (including repeated clinical courses)</summary><ul className="grid gap-1 sm:grid-cols-2 mt-3">{a.graduateRequirements.map(r=><li key={r.code}>{r.code}: {r.credits} credits across {r.occurrences} enrollment{r.occurrences===1?'':'s'} · {r.completed?'Completed':r.covered?'Planned':'Needs scheduling'}</li>)}</ul></details>
